...
def askForAssignmentMarks(grades, gnahatakanner):
    current_grades = {"mygrades": {}}

    for key in grades:
        try:
            if gnahatakanner[key] > -1:
                print "Your Grade from " + key + " is " + str(gnahatakanner[key])
                current_grades["mygrades"][key] = gnahatakanner[key]
...
def main():
    ...
    current_grades = askForAssignmentMarks(grades, gnahatakanner)
